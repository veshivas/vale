...
// TestGetQDocProseBlocksDesignersFile reads qt-edu-for-designers.qdoc and
// checks that prose after the second \section1 lands on the correct source lines.
func TestGetQDocProseBlocksExternalFile(t *testing.T) {
	const filePath = "/Users/veshivas/gitrepos/_qt6/qtdoc/doc/edu/src/qt-edu-for-designers.qdoc"
	...
	// comment.Line=4, so prose_line = source_line - 3.
	// "were installed" at source line 40 → prose line 37.
	// "is located" at source line 41 → prose line 38.
	want := map[string]int{
		"were installed": 37,
		"is located":     38,
	}
		...
		for _, blk := range blocks {
			...
			for i, l := range lines {
				if strings.Contains(l, substr) {
					gotLine := i + 1
					if gotLine != wantLine {
						t.Errorf("prose %q: prose line %d, want %d\nprose text (lines 35-45):\n%s",
							substr, gotLine, wantLine,
							strings.Join(lines[max(0, 34):min(45, len(lines))], "\n"))
					}
					found = true
					break
				}
